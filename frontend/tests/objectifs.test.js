import { ajouterObjectifs } from '../src/pages/Objectifs'
...
test("Check si l'objectif est déjà présent sur le profil", function() {
    const obj = ajouterObjectifs(objectifsTemplateCourir)
    expect(obj).toStrictEqual(["Apprendre l'anglais", "Courir plus"]);
})

test('Erreur si objectif existe déjà', function() {
    const obj = ajouterObjectifs(objectifsTemplateAnglais)
    expect(obj).toBe("L'objectif a déjà été ajouté")
})

test("Erreur si pas d'objectif introduit", function(){
    const obj = ajouterObjectifs(objectifVoid)
